...
wire inst_lbu, inst_lb, inst_lw, inst_sb, inst_sw;
assign {inst_lbu, inst_lb, inst_lw, inst_sb, inst_sw} = lsu_op;

wire access_mem = |lsu_op;

assign addr_o = {32{access_mem}} & addr_i;
assign RDEN = inst_lbu | inst_lb | inst_lw;
...
assign rdata_o = {32{inst_lw}} & rdata_i | 
    ...
    {32{inst_lbu}} & {24'b0, shift_rdata[7:0]};

//-------------------------------------------store---------------------------------------------------//

wire byte_at_00 = inst_sb & (~addr_i[1]) & (~addr_i[0]);
wire byte_at_01 = inst_sb & (~addr_i[1]) &   addr_i[0];
wire byte_at_10 = inst_sb &   addr_i[1]  & (~addr_i[0]);
wire byte_at_11 = inst_sb &   addr_i[1]  &   addr_i[0];
wire word_at_00 = inst_sw & (~addr_i[1]) & (~addr_i[0]);    
