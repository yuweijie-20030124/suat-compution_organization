...
def case_lb(rd, offset, rs1=DRAM_BASE_REG):
    return enc_i(offset, rs1, LOAD_OPS["lb"], rd, 0x03), f"lb {x(rd)},{offset}({x(rs1)})"


def case_lw(rd, offset, rs1=DRAM_BASE_REG):
    return enc_i(offset, rs1, LOAD_OPS["lw"], rd, 0x03), f"lw {x(rd)},{offset}({x(rs1)})"


def case_lbu(rd, offset, rs1=DRAM_BASE_REG):
    return enc_i(offset, rs1, LOAD_OPS["lbu"], rd, 0x03), f"lbu {x(rd)},{offset}({x(rs1)})"


def case_sb(rs2, offset, rs1=DRAM_BASE_REG):
    return enc_s(offset, rs2, rs1, STORE_OPS["sb"]), f"sb {x(rs2)},{offset}({x(rs1)})"


def case_sw(rs2, offset, rs1=DRAM_BASE_REG):
    return enc_s(offset, rs2, rs1, STORE_OPS["sw"]), f"sw {x(rs2)},{offset}({x(rs1)})"
...
def directed_data_cases():
    ...
    cases = [
        # Catch sb clobbering other bytes and sw not writing
        case_addi(11, 0, 0),
        case_sw(11, 16), # clear the word at 16 to be 0
        case_addi(11, 0, -1),
        case_sb(11, 16),
        case_lbu(12, 17), # should be 0 because sb at 16 should only write to offset 16
        case_sw(11, 20),
        case_lw(13, 20),  # should be -1 because sw should write full word
...
def rand_rd(rng):
    ...
    # x2 is reserved as DRAM_BASE_ADDR for memory safety.
    if rng.randrange(8) == 0:
        return 0
    return rng.choice([reg for reg in range(3, 32)])
...
def make_case(op, rng, pc_offset):
    ...
    if op in R_OPS:
        ...
        rs1 = DRAM_BASE_REG
        rs2 = rng.choice([0] + list(range(3, 32)))
...
def verify_cases(cases):
    ...
    for idx, (word, asm) in enumerate(cases):
        ...
        if op in BRANCH_OPS or op == "jal":
            ...
            addr = DRAM_BASE_ADDR + imm
            width = 4 if op in ("lw", "sw") else 1
            if not (DRAM_BASE_ADDR <= addr <= DRAM_BASE_ADDR + DRAM_SIZE - width):
...
def main():
    ...
    directed_ctrl = directed_control_flow_cases(start_idx=2 + len(directed_data))
    directed = directed_data + directed_ctrl
    body_count = COUNT - 3 - len(directed)
    if body_count < 0:
    ...
    cases = [
        (enc_u(BASE_ADDR >> 12, BASE_REG, 0x37), f"lui x{BASE_REG},0x{BASE_ADDR >> 12:x}"),
        (enc_u(DRAM_BASE_ADDR >> 12, DRAM_BASE_REG, 0x37), f"lui x{DRAM_BASE_REG},0x{DRAM_BASE_ADDR >> 12:x}")
    ]
    ...
    lines = [
        ...
        "# safe constraints: x1 is 0x80000000, x2 is 0x80010000; load/store offsets stay inside 64KB DRAM",
