...
`ifndef TEST_ROOT
`define TEST_ROOT "yourpath/lab6"
`endif
...
module testbench;
    ...
    task compare_commit;
        ...
        begin
            ...
            if (verbose != 0) begin
                $display("[DIFFTEST] commit=%0d retired_pc=0x%08x",
                    idx + 1, commit_retired_pc_sample);
            end

            check32(idx, "retired_pc", exp_retired_pc, commit_retired_pc_sample);
            stop_on_error(idx, error_count_before, exp_retired_pc, exp_next_pc);
    ...
    task dump_fail_trace;
        ...
        begin
            ...
            $display("[DIFFTEST][TRACE] GOLD retired_pc=0x%08x next_pc=0x%08x",
                exp_retired_pc, exp_next_pc);
            $display("[DIFFTEST][TRACE] RTL  retired_pc=0x%08x next_pc=0x%08x",
                commit_retired_pc_sample, commit_next_pc_sample);
            ...
            $display("[DIFFTEST][TRACE] RTL  mem valid=%b addr16=0x%04x rdata=0x%08x wdata=0x%08x wren=0x%x",
                u_top.ex_mem_valid, u_top.ls_addr[15:0], u_top.mem_rdata,
            ...
            $display("[DIFFTEST][TRACE] queue fetch_head=%0d fetch_tail=%0d",
