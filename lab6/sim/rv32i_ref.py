...
def decode(inst: int) -> DecodedInst:
    ...
    if opcode == 0x03:
        names = {0x0: ("lb", 1), 0x2: ("lw", 4), 0x4: ("lbu", 1)}
        if funct3 in names:
...
def run_model(
    ...
    cpu = Rv32Ref(program=program, base=base, iram_size=iram_size, dram_size=dram_size, dram_base=dram_base)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--dram-base", type=lambda s: int(s, 0), default=0x80010000)
...
def main() -> None:
    ...
    trace = run_model(
        ...
        max_steps=args.max_steps,        dram_base=args.dram_base,    )
